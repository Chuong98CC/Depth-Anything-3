...
def approximate_depth_from_grayscale(gray_img: np.ndarray, max_depth_m: float = 5.0) -> np.ndarray:
    """Approximate the original depth in metres from a grayscale depth image.

    This reverses the log-normalization used by the original conversion script:
      grey = 30 + normalized * 210
      normalized = log1p(depth_m)/log1p(max_depth_m)

    Args:
        gray_img: (H, W) uint8 array of pixel values in [0, 255]; zero means invalid.
        max_depth_m: the far-plane clip distance in metres used for normalization.

    Returns:
        (H, W) float32 array of approximate depths in metres, 0.0 for invalid pixels.
    """
    # If the input is a 3-channel image (e.g. JPEG loaded as BGR), take the first
    # channel since all channels are equal for a greyscale depth visualization.
    if gray_img.ndim == 3:
        gray_img = gray_img[:, :, 0]
    ...
    normalized = (gray_f - 30.0) / 210.0
    normalized = np.clip(normalized, 0.0, 1.0)

    depth_m = np.expm1(normalized * np.log1p(max_depth_m))
